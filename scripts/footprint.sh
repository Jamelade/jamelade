...
#
# What Jamelade costs the machine it runs on: memory, CPU, and disk.
#
# There is no single number, and the obvious one is wrong. Jamelade is a Rust
...
# for having it.
#
#   scripts/footprint.sh          # measure a running Jamelade
#   scripts/footprint.sh --disk   # disk only, no instance needed
#
# Start Jamelade first. Play something before measuring if you want the playing
# figure — a decoder that has never decoded is not the working set.

set -uo pipefail
cd "$(dirname "$0")/.."

BUS=io.github.Jamelade.Jamelade.Launcher
...
disk() {
	...
	local rows=(
		"the app itself|target/release/jamelade"
		"the sidecar (Chromium)|sidecar/node_modules"
		"Widevine CDM + session|$HOME/.config/Jamelade"
		"artwork + library cache|$HOME/.cache/jamelade"
		"last session|${XDG_STATE_HOME:-$HOME/.local/state}/jamelade"
	)
...
# **Ask the bus, never scan /proc.** An instance started as ./target/debug/jamelade
...
if [ -z "$PID" ] || [ ! -d "/proc/$PID" ]; then
	echo "Jamelade is not running — start it first, or use --disk." >&2
	exit 1
fi
...
echo "  desktop twice (#37, and the edge-versus-level rule in ARCHITECTURE.md)."
echo
