...
#
#   1. Rust must not drive the queue. `RUST_LOG=jamelade=info` prints a
...
# Which sink-input belongs to Jamelade. Matched on any property containing the
...
# `app.setName('Jamelade')` in sidecar/main.js is what puts it there at all.
#
...
jamelade_indices() {
	pactl list sink-inputs 2>/dev/null | awk '
		...
		tolower($0) ~ /jamelade/ { hit = 1 }
		/^[[:space:]]*$/ { if (hit && idx != "") print idx; idx = ""; hit = 0 }
		END { if (hit && idx != "") print idx }
	'
}

echo "Watching Jamelade's audio stream. Play across a track boundary."
echo "Expect: nothing at all. A 'remove' of Jamelade's own stream is a gap."
echo
printf 'Timestamps below are local time; the app logs UTC (offset %s).\n' \
	"$(date '+%:z')"
echo

known=$(jamelade_indices | tr '\n' ' ')
if [ -n "${known// /}" ]; then
	echo "Jamelade is already playing on sink-input(s): $known"
else
	echo "Jamelade is not playing yet — its stream will appear when you hit play."
fi
echo
...
pactl subscribe 2>/dev/null | while read -r line; do
	...
	*remove*)
		...
		case " $known " in
		*" $index "*)
			echo "$stamp  Jamelade's stream #$index went away"
		...
		;;
	*)
		current=$(jamelade_indices | tr '\n' ' ')
		case " $current " in
		*" $index "*)
			[ -n "${known// /}" ] || echo "$stamp  Jamelade's stream #$index appeared"
			known=$current
			;;
		esac
		;;
