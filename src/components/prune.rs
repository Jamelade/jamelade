...
//! Keeping `~/.cache/jamelade/artwork` from growing forever.
//!
//! The Chromium HTTP cache is disabled and the library cache is one bounded
//! file, but every cover fetched for every search anybody ever ran otherwise
//! stays on disk. Measured at 973 files and 41MB against a library that can
//! only account for about 700 of them.
//!
...
pub(super) fn classify(name: &str) -> Entry<'_> {
    ...
    // `<key>-512.backdrop256-g2-r10.png` — a blurred copy of the cover beside
    // it, keyed by renderer and slider radius, and it lives or dies with that
    // cover. The numberless `backdrop.png` form predates cache versioning and
    ...
    // `<key>-512.palette1` — two RGB values extracted from the same cover for
    // album-coloured glass. It is derived, tiny and useless once its cover is
    // evicted, so it follows exactly the same keep-set decision.
    if let Some(size) = rest.strip_suffix(".palette1") {
        return if LIVE_SIZES.contains(&size) {
            Entry::Cover(key)
        } else {
            Entry::Retired
        };
...
pub fn run(keep: &HashSet<String>) -> Report {
    ...
    // Never traverse a substituted cache-directory symlink. Pruning deletes
    // by filename, so following one could turn a same-user cache corruption
    // into deletion inside an unrelated directory.
    ...
    let files: Vec<(PathBuf, u64)> = entries
        .flatten()
        .filter_map(|e| {
            // `file_type` does not follow symlinks. A cache entry pretending to
            // be a cover must never make us inspect or remove its target.
    ...
    for (path, len) in &files {
        if evictable(path, keep, over_cap) {
            match std::fs::remove_file(path) {
                ...
                Err(_) => tracing::debug!("could not prune an artwork cache entry"),
            }
        } else {
            report.kept += 1;
        }
...
mod tests {
    ...
    fn every_shape_on_disk_is_recognised() {
        ...
        assert_eq!(
            classify("127f514fc028ac76-512.backdrop256-g2-r10.png"),
            Entry::Cover("127f514fc028ac76")
        );
        assert_eq!(
            classify("127f514fc028ac76-512.palette1"),
            Entry::Cover("127f514fc028ac76")
        );
    ...
    fn a_backdrop_follows_the_cover_it_was_blurred_from() {
        ...
        assert_eq!(
            classify("abcdef0123456789-512.backdrop256-g2-r10.png"),
    ...
    fn the_shapes_we_stopped_writing_are_retired() {
        ...
        assert_eq!(classify("abcdef0123456789-256.palette1"), Entry::Retired);
    }
