...
/// "Build the fallback picture for this artless playlist."
pub type PlaylistArtRequest = Rc<dyn Fn(String, String, Vec<Artwork>)>;

/// Registry/cache identity for a playlist collage without exposing its id.
pub fn playlist_art_key(playlist: &Playlist) -> String {
    let mut hash: u64 = 0xcbf2_9ce4_8422_2325;
    let mut mix = |byte| {
        hash ^= u64::from(byte);
        hash = hash.wrapping_mul(0x0000_0100_0000_01b3);
    };
    for byte in playlist
        .id
        .bytes()
        .chain(std::iter::once(0))
        .chain(playlist.last_modified.bytes())
    {
        mix(byte);
    }
    for artwork in &playlist.mosaic_artwork {
        mix(0);
        for byte in artwork.cache_key().bytes() {
            mix(byte);
        }
    }
    format!("playlist-{hash:016x}")
}
...
impl Tile {
    ...
    fn artless_playlist(&self) -> Option<&Playlist> {
        match self {
            Self::Playlist(playlist) if playlist.artwork.is_none() => Some(playlist),
            Self::Album(_) | Self::Artist(_) | Self::Playlist(_) => None,
        }
    }
...
pub struct GridItem {
    ...
    request: ArtRequest,
    playlist_request: PlaylistArtRequest,
}
...
impl RelmGridItem for GridItem {
    ...
    fn bind(&mut self, widgets: &mut Self::Widgets, _root: &mut Self::Root) {
        ...
        *widgets.showing.borrow_mut() = match &self.tile {
        ...
        } else if let Some(playlist) = self.tile.artless_playlist() {
            let key = playlist_art_key(playlist);
            (self.playlist_request)(
                key.clone(),
                playlist.id.clone(),
                playlist.mosaic_artwork.clone(),
            );
            register(&mut self.registry.borrow_mut(), key, &widgets.cover);
        }
    }
...
mod tests {
    ...
    #[test]
    fn a_playlist_edit_invalidates_its_collage_key() {
        let playlist = |modified: &str| Playlist {
            id: "p.private-id".into(),
        ...
        let mut with_cover = playlist("before");
        with_cover.mosaic_artwork = vec![Artwork::new("https://is1.mzstatic.com/a/{w}x{h}bb.jpg")];

        assert_ne!(before, after);
        assert_ne!(before, playlist_art_key(&with_cover));
        assert!(!before.contains("private-id"), "library id leaked into key");
    }
}
