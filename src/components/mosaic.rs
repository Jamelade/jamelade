...
use super::artwork::{cache_dir, fetch, is_safe_cached_file, write_atomically};
...
/// and written up in ARCHITECTURE.md — so unlike everything else in this module there
...
pub async fn mosaic(arts: Vec<Artwork>, size: u32, tile_px: u32) -> Option<PathBuf> {
    ...
    if is_safe_cached_file(&out, 10 * 1024 * 1024) {
        return Some(out);
    }
    ...
    while let Some(joined) = tasks.join_next().await {
        ...
        match fetched {
            ...
            Err(_) => {
                tracing::warn!("mosaic tile not fetched; no mosaic");
                return None;
            }
        }
    }
    ...
    if write_atomically(&out, &bytes).is_err() {
        tracing::warn!("mosaic not written");
        return None;
    }
    tracing::debug!(
        tiles = tiles.len(),
...
fn compose(covers: &[PathBuf], size: i32) -> Option<Vec<u8>> {
    ...
    canvas
        .save_to_bufferv("png", &[])
        .map_err(|_| tracing::warn!("mosaic not encoded"))
        .ok()
}
