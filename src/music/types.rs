...
//! Our types. Apple's JSON stops here (ARCHITECTURE.md rule 9) — "parse, don't
...
pub struct Playlist {
    ...
    pub artwork: Option<Artwork>,
    /// Album covers used when Apple supplies no playlist artwork of its own.
...
///
/// Stations are not ordinary finite song queues, so they deliberately carry
/// only the descriptor MusicKit needs to start one. Once it is playing,
/// MusicKit's queue events remain the source of truth like every other queue.
#[derive(Debug, Clone, PartialEq)]
pub struct Station {
    pub id: String,
    pub name: String,
...
/// One thing Apple can put on the Explore page.
///
/// Kept deliberately narrower than Apple's generic `Resource`: Jamelade knows
/// how to open albums and playlists, queue songs, and hand stations back to
/// MusicKit. Music videos remain ignored at the JSON boundary rather than
/// becoming cards that cannot do anything when clicked.
#[derive(Debug, Clone, PartialEq)]
...
impl ExploreItem {
    ...
    pub fn subtitle(&self) -> &str {
        match self {
            ...
            Self::Playlist(playlist) => &playlist.curator,
            Self::Track(track) => &track.artist,
            Self::Station(_) => "Apple Music Radio",
        }
    }

    pub fn artwork(&self) -> Option<&Artwork> {
        match self {
            Self::Album(album) => album.artwork.as_ref(),
            Self::Playlist(playlist) => playlist.artwork.as_ref(),
            Self::Track(track) => track.artwork.as_ref(),
            Self::Station(station) => station.artwork.as_ref(),
        }
    }
}

/// A bounded horizontal shelf on Explore.
#[derive(Debug, Clone, PartialEq)]
pub struct ExploreSection {
    pub title: String,
    /// Apple's localized reason for a recommendation, when it supplied one.
    pub subtitle: String,
    pub items: Vec<ExploreItem>,
}

/// Everything needed to render Explore, already separated from Apple's wire
/// shapes. The UI never receives a heterogeneous JSON resource.
...
#[derive(Debug, Clone, Deserialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct StationAttributes {
    #[serde(default)]
    pub name: String,
    pub artwork: Option<ArtworkAttributes>,
}

impl From<Resource<StationAttributes>> for Station {
    fn from(res: Resource<StationAttributes>) -> Self {
        let attributes = res.attributes;
        Self {
            id: res.id,
            name: attributes
                .as_ref()
                .map(|attributes| attributes.name.clone())
                .unwrap_or_default(),
            artwork: attributes
                .and_then(|attributes| attributes.artwork)
                .filter(|artwork| !artwork.url.is_empty())
                .map(|artwork| Artwork::new(artwork.url)),
        }
    }
}
...
mod tests {
    ...
    fn artwork_survives_a_round_trip_with_its_cache_key_intact() {
        ...
        // template, every cover in `~/.cache/jamelade/artwork` would miss and
