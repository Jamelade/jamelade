...
impl AppModel {
    ...
    pub(super) fn push_snapshot(&self) {
        ...
        let playing = self.player.state.is_playing();
        self.lyrics_view.set_playing(playing);
        self.jamkin_mode.set_playing(playing);
    ...
    pub(super) fn sync_artwork(&mut self, sender: &ComponentSender<Self>) -> bool {
        ...
        match template {
            Some(t) => {
                let art = Artwork::new(t.clone());
                let glass_strength = self.settings.glass_strength;
                ...
                sender.oneshot_command(async move {
                    ...
                    // in the same message: the cover, backdrop and glass
                    // palette taken from it must never be applied a frame apart.
                    let backdrop = path
                        .as_deref()
    ...
    /// Re-render the current cover for a changed glass setting. This is the
    /// image-processing half of the Preferences slider; CSS opacity has already
    /// moved by the time this command is queued.
    pub(super) fn refresh_backdrop(&self, sender: &ComponentSender<Self>) {
        let Some(source) = self.art_path.clone() else {
            return;
        };
        let glass_strength = self.settings.glass_strength;
        let generation = self.account_generation;
