...
impl AppModel {
    ...
    pub(super) fn drop_removed_track(&mut self, catalog_id: &str) {
        ...
        tracing::info!(removed_row = row.is_some(), "track left the library");
    }
    ...
    pub(super) fn settle_library_write(&mut self, kind: &str, id: &str, ok: bool, detail: &str) {
        ...
        let Some(pending) = self.pending_writes.remove(id) else {
            tracing::debug!(kind, ok, "library write with no pending record");
            return;
        };
        ...
        // The renderer's detail is needed only to correlate recovery. It may
        // contain remote text, so neither it nor the resource id belongs in a
        // persistent log.
        let _ = detail;
        tracing::warn!(kind, "library write refused; row put back");
        match pending.undo {
