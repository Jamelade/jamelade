...
//! The entire sidecar contract, in one file (ARCHITECTURE.md rule 9).
//!
...
pub enum Command {
    ...
    /// Start an Apple Music radio station. A station is an open-ended MusicKit
    /// queue descriptor, not a finite list Jamelade can manufacture itself.
...
pub enum Event {
    ...
    /// Whether Chromium could bind cookie encryption to the desktop keyring.
    /// False means the sidecar deliberately chose a memory-only partition.
...
fn storefront<'de, D: serde::Deserializer<'de>>(de: D) -> Result<String, D::Error> {
    ...
    if value.len() == 2 && value.bytes().all(|byte| byte.is_ascii_lowercase()) {
        ...
        Err(serde::de::Error::custom("invalid storefront"))
    }
}
...
pub struct Queue {
    ...
    pub position: i64,
    #[serde(default, deserialize_with = "bounded_queue_items")]
    pub items: Vec<Item>,
}

const MAX_QUEUE_ITEMS: usize = 2_500;

fn bounded_queue_items<'de, D>(de: D) -> Result<Vec<Item>, D::Error>
where
    D: serde::Deserializer<'de>,
{
    let items = Vec::<Item>::deserialize(de)?;
    if items.len() > MAX_QUEUE_ITEMS {
        return Err(serde::de::Error::custom(
            "MusicKit queue exceeded the safe item limit",
        ));
    }
    Ok(items)
}
...
mod tests {
    ...
    #[test]
    fn an_oversized_renderer_queue_is_rejected() {
        let items = std::iter::repeat_n("{}", MAX_QUEUE_ITEMS + 1)
            .collect::<Vec<_>>()
            .join(",");
        let raw = format!(r#"{{"items":[{items}]}}"#);
        assert!(serde_json::from_str::<Queue>(&raw).is_err());
    }
    ...
    #[test]
    fn a_station_uses_its_own_open_ended_queue_descriptor() {
        let json = serde_json::to_string(&Command::PlayStation {
            station: "ra.1234".into(),
        })
        .unwrap();
        assert_eq!(json, r#"{"cmd":"playStation","station":"ra.1234"}"#);
        assert_eq!(
            Command::PlayStation {
                station: "ra.1234".into()
            }
